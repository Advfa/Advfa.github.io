...
function make_datatable(table_id, pagetitle, livesite, ajaxurl, showTools, pageSize) {
	bProcessing = false;
	bServerSide = false;
	...
	if (showTableTools)
		...
		if (jomres_template_version == "bootstrap3"){
			sDomm = "<'row-no-pd no-fit '<'hide'lr><'col-xs-6 text-left'f><'col-xs-6 text-right'B>>t<'row'<'col-xs-6'i><'col-xs-6'p>>";
			}
		else {
	...
	var oTable = jomresJquery('#' + table_id).dataTable({
		...
		"lengthMenu": [[pageSize, 10, 25, 50, -1], [pageSize, 10, 25, 50, "All"]],
		"sDom": sDomm,
		"order": [[ 0, "desc" ]],
		"searchDelay": 1000,
		"oLanguage": {
				...
				//"sSearch":         dataTables_sSearch,
				"sSearchPlaceholder":         dataTables_sSearch,
...
function jomres_submitbuttonInputHidden(pressbutton) {
    var form = jomresJquery("input:hidden[value='" + pressbutton + "']").closest("form")[0];
